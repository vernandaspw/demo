...
$sql1=mysqli_query($koneksi,"SELECT * from barang_masuk where id_barang_masuk = '$id_barang_masuk'") or die (mysqli_error($koneksi));
$databarang=mysqli_fetch_array($sql1);
?>
<div class="col-md-12">
              <div class="tile">
                <div class="panel-heading">Edit Data Barang Masuk &nbsp; <a href="index.php" class="btn btn-warning btn-sm">Kembali</a></div>
                <div class="panel-body">
                    <form method="post">
                        <div class="form-group">
                            <label>Kode Barang</label>
                            <input type="text" name="kd_barang" value="<?php echo $databarang['kd_barang']; ?>" class="form-control" readonly/>
                        </div>
                        <div class="form-group">
                            <label>Jumlah Masuk</label>
                            <input type="text" name="jumlah_masuk" value="<?php echo $databarang['jumlah_masuk']; ?>" class="form-control"  />
                        </div>
                        <div class="form-group">
                            <label>Tanggal</label>
                            <input type="text" name="tanggal" value="<?php echo $databarang['tanggal']; ?>" class="form-control" readonly />
                            </div>
                            <div class="form-group">
                    ...
                    if(@$_POST['simpan']) {
                        ...
                        if($databarang['jumlah_masuk'] >  $jumlah_masuk){
                          $jumlah    = $stok + ($jumlah_masuk - $databarang['jumlah_masuk']);
                        }else{
                          $jumlah    = $stok + $jumlah_masuk - $databarang['jumlah_masuk'];
                        }
                        mysqli_query($koneksi, "UPDATE barang_masuk SET jumlah_masuk = '$jumlah_masuk' WHERE kd_barang='$kd_barang' ") or die ($koneksi->error);
                        mysqli_query($koneksi, "UPDATE data_barang SET stok = '$jumlah' WHERE kd_barang='$kd_barang' ") or die ($koneksi->error);
