...
$no = 1;
$sql1 = mysqli_query($koneksi, "SELECT * FROM pembayaran inner join transaksi on pembayaran.id_transaksi = transaksi.id_transaksi INNER JOIN data_kustomer where id_pembayaran = '$id_pembayaran'") or die ($koneksi->error);
              ...
              <div class="tile">
                ...
                <div class="panel-body">
                    ...
                    <div class="form-group">
                            ...
                            <input type="text" name="no_faktur" class="form-control" value="<?php echo $databarang['no_faktur']; ?>" readonly />
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <?php
                             $id_pembayaran = @$_GET ['id_pembayaran'];
                             $sql3 =mysqli_query($koneksi, "SELECT * FROM pembayaran inner join transaksi on pembayaran.id_transaksi = transaksi.id_transaksi INNER JOIN data_kustomer where id_pembayaran = '$id_pembayaran'");
                             $data3    =mysqli_fetch_array($sql3);
                             $harga     = $data3['total_bayar'];
                          
                            ?>  
                            <input type="text" name="total_bayar" value="<?php echo $harga ?>" class="form-control" readonly />
                    ...
                    if(@$_POST['simpan']) {
                        ...
                        $sql5 =mysqli_query($koneksi, "SELECT * FROM pembayaran inner join transaksi on pembayaran.id_transaksi = transaksi.id_transaksi INNER JOIN data_kustomer where id_pembayaran = '$id_pembayaran'");
                        $data5    =mysqli_fetch_array($sql5);
                        $jumlah     = $data5['saldo'] - $total_bayar;

                        $tanggal_bayar=date('Y:m:d');
                        $sql1 = mysqli_query($koneksi, "UPDATE data_kustomer SET saldo = '$jumlah' WHERE id_kustomer = '$id_kustomer'");
                        $sql1 = mysqli_query($koneksi, "UPDATE pembayaran SET total_bayar = '$total_bayar', tanggal_bayar = '$tanggal_bayar' WHERE id_pembayaran = '$id_pembayaran'");
                        echo "<script>alert('Berhasil Bayar')</script>";
